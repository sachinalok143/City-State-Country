	...
	<table id="district_result_table" class="table table-striped table-bordered">
		...
		<tr ng-repeat="district in districts|filter:search1|limitTo:rowLimit:startRow|orderBy:sortColumn:reverseSort">
			...
			<td><button class="edit btn btn-primary" id="show_district_form" ng-click="showEditDistrictForm(district.id,district.district_name,district.state_id,district.operating_status);">Edit</button></td>
			<td><button class="edit btn btn-danger" ng-click="deleteDistrict(district.id)">Delete</button></td>				
		</tr>
