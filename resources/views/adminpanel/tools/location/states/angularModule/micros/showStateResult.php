	...
	<table id="state_result_table" class="table table-striped table-bordered">
        ...
        <br><br>
			<tr ng-repeat="state in states|filter:search1|limitTo:rowLimit:startRow|orderBy:sortColumn:reverseSort">
				...
				<td><button class="edit btn btn-primary" id="show_state_form" ng-click="showEditStateForm(state.id,state.name,state.country_id,state.Capital,state.rto_state_code);">Edit</button></td>
				<td><button class="edit btn btn-danger" ng-click="deleteState(state.id)">Delete</button></td>				
			</tr>
	</table>
	<div>
