var myApp= angular.module("Form",['toaster', 'ngAnimate'],function($interpolateProvider) {
...
$scope.search=function(district){
// console.log(district);
$http.post('/Admin/Location/City/search',{
...
if($scope.validate){
	...
	$http.post('/Admin/Location/City/Add',{
		city:$scope.city,
	}).then(function(response){
		// console.log(response.data.cities);
		$scope.cities=response.data.cities;
...
$scope.delete_city=function(id){
	if(confirm('Deleted Date wil not be recoverd! Do you want delete it?')){
$http.post('/Admin/Location/City/delete_city',{
		city:id,
	}).then(function(response){
		$scope.cities=response.data.cities;
		$scope.checkData($scope.cities); 
...
// confirm('aaesrdtu');
$http.post("/Admin/Location/City/update",{
 		 city:$scope.city,
		// console.log($scope.city);	
 	})
 	 .then(function(response) {
