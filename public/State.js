...
$scope.startRow=0;
$scope.rowLimit='';
	$http.get("/Admin/Location/City/state")
...
$scope.Edit_state_form=function(id,name,country_id,Capital,rto_state_code)
{
	$scope.warning='';
...
$http.post("/Admin/Location/City/updatestate",{
 		 state:$scope.state,
...
$scope.delete_state =function(id){
	
	if(confirm('Deleted data will never recovered! Do you want delete it?')){
$http.post('/Admin/Location/City/delete_state',{
		state:id,
	}).then(function(response){
		$scope.states=response.data.states;
		$scope.checkData($scope.states);
...
$scope.add_newState_form=function(){
	$scope.warning='';
	$scope.state={};
	document.getElementById('add_newstate_form').style.display='block';
};

$scope.Add_state_form=function(){
for (var i = 0; i < $scope.countries.length; i++) {
		if($scope.countries[i].id==$scope.state.country_id)
		{
			$scope.country_name=$scope.countries[i].name;
...
if($scope.validate){
	...
	$http.post('/Admin/Location/City/AddState',{
		state:$scope.state,
	}).then(function(response){
		$scope.states=response.data.states;
		$scope.checkData($scope.states);
...
$scope.SearchByCountry=function(){
	...
	$http.post('/Admin/Location/City/searchByCountry',{
