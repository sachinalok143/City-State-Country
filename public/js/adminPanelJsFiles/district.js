var myApp= angular
.module("manageDistrict",['toaster', 'ngAnimate'],function($interpolateProvider) {
        $interpolateProvider.startSymbol('<%');
        $interpolateProvider.endSymbol('%>');
    })
.controller("DistrictController", function ($scope,$http,toaster) {
	$scope.district={
		id:0,
		state_id:0,
		district_name:'',
...
$scope.startRow=0;
	...
	$http.get("/Admin/Location/District/get-all-districts")
 	...
 	$scope.showEditDistrictForm=function(id,name,state_id,operating_status)
{
	$scope.warning='';
...
$scope.updateDistrictDetail=function(){
...
$http.post("/Admin/Location/District/update-district-detail",{
...
if($scope.validate){
	...
	$http.post('/Admin/Location/District/add-district',{
		district:$scope.district,
	}).then(function(response){
		$scope.districts=response.data.districts;
		$scope.checkData($scope.districts);
...
$scope.GetState=function(id){
	// alert('/Admin/Location/District/GetState/'+id);
...
$scope.searchCityByState=function(){

	// console.log($scope.state.id);
	$http.post('/Admin/Location/District/seach-state',{
